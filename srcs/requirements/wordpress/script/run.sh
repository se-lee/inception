if ! wp --allow-root --path='/var/www/html/wordpress/' core is-installed; then
		wp --allow-root --path='/var/www/html/wordpress/' core install -url=$WP_URL --title=$WP_TITLE --admin_user=$WP_ADMIN
		--admin_password=$WP_PASS --admin_email=$WP_AMAIL --skip-email --quiet
